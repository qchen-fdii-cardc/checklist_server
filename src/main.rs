    ...
    serde_json::json!({
        ...
        "paths": {
            ...
            "/api/checklists/search": {
                "get": {"summary": "Search checklists by title, description, or item text", "parameters": [{"name": "q", "in": "query", "schema": {"type": "string"}}], "responses": {"200": {"description": "Search results"}}}
            },
...
async fn list_checklists(State(state): State<AppState>) -> Result<Json<Vec<Checklist>>, AppError> {
    let checklists = state.store.lock().unwrap().list_checklists()?;
    Ok(Json(checklists))
}
...
async fn search_checklists_handler(
    ...
    if q.trim().is_empty() {
        return Ok(Json(state.store.lock().unwrap().list_checklists()?));
    }

    Ok(Json(state.store.lock().unwrap().search_by_term(&q)?))
}
...
async fn export_checklist_handler(
    ...
    Ok((StatusCode::OK, [(axum::http::header::CONTENT_TYPE, "application/json")], json).into_response())
}
...
async fn main() {
    ...
    let docs_for_ui = docs.clone();
    let router = app
        .route("/api/docs", get(move || async move { Json(docs_for_ui) }))
        .route("/api-docs/openapi.json", get(move || async move { Json(docs) }));
